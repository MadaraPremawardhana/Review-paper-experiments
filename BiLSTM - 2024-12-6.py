...
# Step 9: Updated prediction function
def predict_instance(instance, time_steps=5):
    instance = np.array(instance).reshape(1, -1)
    instance_scaled = scaler_X.transform(instance)
    instance_sequence = np.repeat(instance_scaled, time_steps, axis=0).reshape(1, time_steps, -1)
    prediction = model.predict(instance_sequence, verbose=0)
